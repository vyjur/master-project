...
def c_overlap_c(text, text_wo):
    return "OVERLAP" if (" og " in text and len(text) < 20) or " også " in text or ("," in text and len(text_wo) < 5) or text_wo.strip() == '' else "O"

def t_overlap_d(text, text_wo):
    return "OVERLAP" if ((" på " in text or " i " in text or " den " in text or " pga " in text or " tilbake " in text or " pga. ") and len(text_wo) < 30) or text_wo.strip() == '' else "O"
    ...
    def run(self, e_i, e_j, output=False):
        ...
        for sentence in sentences:
            if e_i['Text'] in sentence and e_j['Text'] in sentence:
                ...
                text_without = text_between.replace(e_i['Text'], "").replace(e_j['Text'], "").replace("\n", " ").strip()
                if output:
                    print("Sentence:", sentence)
                    print(f"1:{e_i['Text']} -----", text_between, f"------2:{e_j['Text']}")
                ...
                if e_i['MedicalEntity'] == "CONDITION" and e_j['MedicalEntity'] == "CONDITION":
                    pred = RULES["CONDITION_OVERLAP_CONDITION"](text_between, text_without)
                elif e_j['MedicalEntity'] in ["CONDITION", "TREATMENT"] and (e_i['TIMEX'] is not None or e_i['TIMEX'] != ''):
                    ...
                    pred = pred2
                elif e_i['MedicalEntity'] in ["CONDITION", "TREATMENT"] and (e_j['TIMEX'] is not None or e_j['TIMEX'] != ''):
                    #pred1 = RULES["TREATMENT_BEFORE_DATE"](text_between) if first else "O"
                    pred2 = RULES["TREATMENT_OVERLAP_DATE"](text_between, text_without)
                    ...
                    pred = RULES['TREATMENT_BEFORE_CONDITION'](text_between, text_without) if first else "O"
                elif e_i['MedicalEntity'] == "TREATMENT" and e_j["MedicalEntity"] == "TREATMENT":
                    pred = RULES["TREATMENT_BEFORE_TREATMENT"](text_between, text_without) if not first else "O"
