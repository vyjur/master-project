    ...
    match input_tag_type:
        ...
        case TAGS.XML_TYPE:
            ...
            if start and not single:    
                return str(e['Context']).replace(str(e["Text"]), f'<{cat}A>{e["Text"]}</{cat}A>')
            elif not start and not single:
                return str(e['Context']).replace(str(e["Text"]), f'<{cat}B>{e["Text"]}</{cat}B>')

            return str(e['Context']).replace(str(e["Text"]), f'<{cat}>{e["Text"]}</{cat}>')
        case TAGS.SOURCE:
            if entity and single:
                return str(e['Context']).replace(str(e["Text"]), f'es {e["Text"]} ee')
            elif entity and not single:
                if start:
                    return str(e['Context']).replace(str(e["Text"]), f'eas {e["Text"]} eae')
                else:
                    return str(e['Context']).replace(str(e["Text"]), f'ebs {e["Text"]} ebe')
            elif date:
        ...
        case TAGS.CUSTOM:
            if entity and single:
                ...
                if start:
                    return str(e['Context']).replace(str(e["Text"]), f'as es {e["Text"]} ee ae')
                else:
                    return str(e['Context']).replace(str(e["Text"]), f'bs es {e["Text"]} ee be')
            elif date:
                return str(e['Context']).replace(str(e["Text"]), f'ts {e["Text"]} ts') 
            else:
                return str(e['Context'])
