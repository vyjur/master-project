...
SAVE_DIRECTORY = './src/textmining/ere'
class ERExtract:
        ...
        for k, doc in enumerate(dataset_mer):
            for i, e_i in enumerate(doc.itertuples()):
                for j, e_j in enumerate(doc.itertuples()):
                    if i == j:
                        continue
                    relations = dataset_ere[k][(dataset_ere[k]['fk_id'] == e_i[1]) & (dataset_ere[k]['id'] == e_j[1])]
                    
                    if len(relations) == 1:
                        relation = relations.iloc[0]['Entity Relation']
                    else:
                        relation = 'O'
...
if __name__ == "__main__":
    ...
    reg = ERExtract('./src/textmining/ere/config.ini', manager)
