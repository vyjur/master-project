...
class TEExtract:
    def __init__(
        ...
        if not load:
            ...
            for _, row in raw_dataset.iterrows():
                ...
                dataset.append(
                    {
                        "sentence": convert_to_input(
                            self.input_tag_type, row, True, True
                        ),
                        "relation": row["TIMEX"],
                    }
                )
            ...
            if test_manager:
                ...
                for _, row in raw_test_dataset.iterrows():
                    if (
                        row["Text"].replace(" ", "").isalpha()
                        or "ICD" in row["Context"]
                    ):
                        continue
                    test_dataset.append(
                        {
    ...
    def __model_run(self, data, prob=False):
        ...
        predictions = [
            self.id2label[int(i)] if int(i) in self.id2label else "O" for i in output
        ]
        if prob:
    ...
    def extract_sectime(self, data):
        ...
        for i, row in dct_candidates.iterrows():
            ...
            if dct_output[0][0] == "DCT":
                ...
                sections.append({"index": start, "value": row["value"]})
    ...
    def predict_sectime(self, data, prob=False):
        if data["text"].isalpha():
            return "DATE"
        data = {"Context": data["context"], "Text": data["text"], "TIMEX": True}
    ...
    def batch_predict_sectime(self, datas, prob=False):
        batch_text = []
        for data in datas:
            data = {"Context": data["context"], "Text": data["text"]}
            text = convert_to_input(self.input_tag_type, data, True, True)
    ...
    def __get_window(self, full_text, timex_text, window_size=50):
        ...
        window = (
            " ".join(before_window) + " " + timex_text + " " + " ".join(after_window)
        )
        return window
...
if __name__ == "__main__":
    ...
    output = tee.run(
        "kl. 13.00. boka er på skolen 12.10 funker fint, men den 23.12 kl. 12.00 funker ikke. Hva med kl.14.04"
    )
