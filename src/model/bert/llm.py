### Note: This file is currently not in use. Bugs may exist.
...
class LLM:
    def __init__(
        ...
        self.__pipeline = pipeline(
            "text-generation", model="meta-llama/Llama-3.1-8B-Instruct", device=device
        )
    ...
    def __process(self, input, output):
        ...
        test = self.tokenizer(
            input, max_length=512, truncation=True, return_offsets_mapping=True
        )  # type: ignore
    ...
    def predict(self, data):
        ...
        for val in data:
            ...
            output = self.__pipeline(
                PROMPTS[1].format(self.__tags_text, val), max_new_tokens=512
            )
...
if __name__ == "__main__":
    ...
    for example in d["examples"]:
        entities = [
            (annot["start"], annot["end"], annot["value"], annot["tag_name"])
            for annot in example["annotations"]
        ]

        dataset_sample.append({"text": example["content"], "entities": entities})
