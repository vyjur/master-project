                ...
                if step != "MER":  
                    ...
                    for _, te in tee_output.iterrows():
                        ...
                        if te['value'] == dct['value']:
                            continue
                        entities.append(
                            Node(te['text'], te['type'], te['dct'], te['context'], self.__normalize_date(te['value']))
                        )
                    ...
                    if not dct_cp:
                        for i, e_i in enumerate(entities):
                            ...
                            for j, e_j in enumerate(entities):
                                if not (isinstance(e_i.type, ME) and isinstance(e_j.type, TIMEX)):
                                    continue
                                ...
                                if relation != "O":
                                    ...
                                    if rel.tr == TLINK.OVERLAP:
                                        if isinstance(e_i.type, ME) and isinstance(e_j.type, TIMEX):
                                            if (prob >= e_i.prob or e_i.date is None) and e_j.date is not None:
                                                e_i.date = e_j.date
                                                e_i.prob = prob
                        ...
                        for cat in dcts:
                            for i, e_i in enumerate(dcts[cat]):
                                for j, e_j in enumerate(dcts[cat]):
                                    ...
                                    if i == j or e_j.value not in e_i.context or not (isinstance(e_i.type, ME) and isinstance(e_j.type, TIMEX)):
                                        continue
